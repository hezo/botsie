        ...
        request('http://worldoftanks.eu/community/accounts/api/1.1/?source_token=WG-WoT_Assistant-1.2.2&search='+playerName+'&offset=0&limit=1', function(error, response, body) {
        	try {
	            if (!error && response.statusCode == 200) {
	                ...
	                var playerid = player.data.items[0].id;
	                request('http://worldoftanks.eu/community/accounts/'+playerid+'/api/1.3/?source_token=WG-WoT_Assistant-1.2.2', function(error, response, body) {
	                    if (!error && response.statusCode == 200) {
